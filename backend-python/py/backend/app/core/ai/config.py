...
AI_CONFIG = {
    ...
    "diagnosis": {
        ...
        [분석 지침]
        1. **aiComment (종합 총평)**: 매뉴얼에 근거하여 사용자의 현재 상태를 분석하고, 발견된 취약점에 대해 전문가 의견을 3~4문장으로 작성하세요.
        2. **top3Types (주의가 필요한 범죄 유형 TOP3)**: 답변과 매뉴얼 내용을 바탕으로 이 사용자가 가장 주의해야 할 피싱/스미싱 수법 3가지를 리스트 형태로 뽑아주세요.
        3. **recommendations (맞춤형 권장사항)**: 매뉴얼에서 제시하는 정확한 대응 수칙을 바탕으로, 사용자가 즉시 실천해야 할 행동 가이드 3가지를 리스트 형태로 제시하세요.
        ...
        {{
          ...
          "top3Types": ["유형1", "유형2", "유형3"],
          "recommendations": ["매뉴얼 기반 권장사항1", "매뉴얼 기반 권장사항2", "매뉴얼 기반 권장사항3"]
        }}
        """,
        "basic_prompt": """
