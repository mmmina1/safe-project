...
function CommunityDetail() {
  ...
  const fetchComments = async () => {
    try {
      const res = await communityApi.getComments(postId, currentUserId);
  ...
  const fetchData = async () => {
    try {
      const postData = await communityApi.getPostDetail(postId, currentUserId);
      setPost(postData);
      setPostLiked(postData.isLiked || postData.liked || false); // 게시글도 이름 둘 다 체크
      setPostLikeCount(postData.likeCount || 0);
      await fetchComments();
    } catch (err) {
      console.error(err);
    } finally {
        ...
        <div className="detail-meta">
          ...
          <span
            ...
            style={{
              ...
              alignItems: "center",
              gap: "4px",
              color: postLiked ? "#ff4d4f" : "#666",
            }}
          >
            {postLiked ? "❤️" : "🤍"} {postLikeCount}
          </span>
        </div>
      </div>
      ...
      <div className="comment-section">
        ...
        <div className="comment-list-container">
          {comments.map((c) => {
            ...
            // 핵심: 서버에서 isLiked로 주든 liked로 주든 둘 다 체크함
            const likedByMe = c.isLiked === true || c.liked === true;
                ...
                <div className="comment-item-header">
                  ...
                  <div className="comment-header-right">
                    ...
                    <div className="comment-owner-btns">
                      {!isEditing && (
                        <button className="btn-reply" onClick={() => {setReplyingToId(cId); setReplyContent("");}}>답글</button>
                      )}
                      {isOwner && !isEditing && (
                        <>
