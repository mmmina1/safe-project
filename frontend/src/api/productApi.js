...
export async function uploadMainImage(productId, file) {
    ...
    try {
        const res = await axiosInstance.post(`/products/${productId}/main-image`, formData, {
            headers: {
...
export async function getMainImage(productId) {
    try {
        const res = await axiosInstance.get(`/products/${productId}/main-image`);
        return res.data; // { "url": "..." }
    } catch (error) {
...
export async function deleteMainImage(productId) {
    try {
        const res = await axiosInstance.delete(`/products/${productId}/main-image`);
